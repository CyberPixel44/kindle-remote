-- Original by https://github.com/bneo99
...
function HttpRemote:onRequest(host, port)
    ...
    return function(data, id_frame)
        ...
        elseif params_arr["action"] == "prevpage" then
            this:turnPage(-1)
        elseif params_arr["action"] == "increasebrightness" then
            this:changeBrightness("IncreaseFlIntensity")
        elseif params_arr["action"] == "decreasebrightness" then
            this:changeBrightness("DecreaseFlIntensity")
        elseif params_arr["action"] == "togglefrontlight" then
            this:toggleSetting("ToggleFrontlight")
        elseif params_arr["action"] == "togglenightmode" then
            this:toggleSetting("ToggleNightMode")
        elseif params_arr["action"] == "fullrefresh" then
            this:refreshScreen()
        else
...
<html><body>
<p>
<a href="/?action=prevpage&loadpage=1">Previous Page</a>
<a href="/?action=nextpage&loadpage=1">Next Page</a>
<a href="/?action=increasebrightness&loadpage=1">Increase Brightness</a>
<a href="/?action=decreasebrightness&loadpage=1">Decrease Brightness</a>
<a href="/?action=togglefrontlight&loadpage=1">Toggle Frontlight</a>
<a href="/?action=togglenightmode&loadpage=1">Toggle Dark Mode</a>
<a href="/?action=fullrefresh&loadpage=1">Refresh Screen</a>
</p>
</body></html>
            ]]
        end
...
-- link for all supported actions

-- brightness actions
function HttpRemote:changeBrightness(action)
    logger.dbg("HttpRemote: Sent event " .. action .. " 1")
    self.ui:handleEvent(Event:new(action, 1))
    self.ui:handleEvent(Event:new("ShowFlDialog"))
end

-- setting actions (frontlight, nightmode)
function HttpRemote:toggleSetting(action)
    self.ui:handleEvent(Event:new(action))
end

function HttpRemote:refreshScreen()
    self.ui:handleEvent(Event:new("FullRefresh"))
end

function HttpRemote:addToMainMenu(menu_items)
    menu_items.httpremote = {
        text = _("HTTP Remote Control"),
        sorting_hint = ("tools"),
        sub_item_table = {
            {
                text_func = function()
